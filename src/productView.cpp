...
void productView::on_tableWidget_products_itemSelectionChanged()
{
    ...
    if(ret) {
        ...
        if(ret) ui->label_provider->setText( InfoProvider() );
...
void productView::on_paintPrinterListingStock(QPrinter *printer)
{
   ...
   // double coeff = paperRect.width() / 21.0; // A4
    ...
    QString sDateTime = QDateTime::currentDateTime().toString(tr("dd-MM-yyyy HH:mm:ss"));
    // list all products
    for(int pIndex=0, page=1, itemPrinted=0; itemPrinted<itemsToPrint ;page++){
        ...
        /// Header
        //CODE
        rect = fm.boundingRect(mLeft+5,rect.top(), wUtil/8,0, Qt::AlignLeft, tr("Codes") );
        painter.drawText( rect, tr("Codes"));
        //NOM
        rect = fm.boundingRect(mLeft+5+wUtil/8,rect.top(), wUtil/3,0, Qt::AlignLeft, tr("Noms")  );
        painter.drawText( rect, tr("Noms") );
        //CATEGORY
        rect = fm.boundingRect(mLeft+5+(wUtil/8)+(wUtil/3),rect.top(), wUtil/5,0, Qt::AlignLeft, tr("Cat\351gories") );
        painter.drawText( rect, tr("Cat\351gories"));
        //STOCK
        rect = fm.boundingRect(mLeft+5+(wUtil/8)+(wUtil/3)*2,rect.top(), wUtil/10,0, Qt::AlignLeft, tr("Stock") );
        painter.drawText( rect, tr("Stock") );
        //STOCK ALERT
        rect = fm.boundingRect(mLeft+5+(wUtil/8)+(wUtil/3)*2+(wUtil/10),rect.top(), wUtil/10,0, Qt::AlignLeft, tr("Stock Alerte") );
        ...
        //Num de page
        textInfo = " - " + tr("Page ") + QString::number(page) + " - ";
        rect = fm.boundingRect(mLeft, pageRect.height() - mBottom, pageRect.width() - (mLeft+mRight), 0, Qt::AlignVCenter |Qt::AlignRight, textInfo );
        rect.translate( 0, -rect.height());
        painter.drawText( rect, Qt::AlignCenter, textInfo);
...
void productView::on_paintPrinterInfoProduct(QPrinter *printer)
{
   ...
   // double coeff = paperRect.width() / 21.0; // A4
    ...
    painter.setFont(font);
    fm= painter.fontMetrics();
    QRect rect = QRect(mLeft, mTop, pageRect.width() - (mLeft+mRight),
...
*/
float productView::getSelectedProductPrice(){
    int m_index_tab = ui->tableWidget_products->currentRow();
     //Si index < 0 on sort
