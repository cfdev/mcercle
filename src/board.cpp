...
void board::listRevenuesToTable()
{
    ...
    //Style de la table produit
    ui->tableWidget_revenue->setColumnCount(2);
    ...
    for(int i=1,j=0; i<13;i++){
        ...
        item_MonthRevenue->setData(Qt::DisplayRole,
                                   m_data->m_customer->m_invoice->getMonthRevenue(ui->comboBox_yearsList->currentText(),
                                                                                  QString::number(i)));
        ...
        ui->tableWidget_revenue->setItem(j, 1, item_MonthRevenue);
        j++;
    }
}
...
void board::calculYear(){
    ...
    float tt = m_data->m_customer->m_invoice->getYearRevenue( ui->comboBox_yearsList->currentText() );
