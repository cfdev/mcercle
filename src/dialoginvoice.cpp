    ...
    ui(new Ui::DialogInvoice)
{
    ...
    if(m_DialogState == EDIT){
        ...
        loadValues();
        if(m_DialogType == PROPOSAL_TYPE)   listProposalDetailsToTable("", "");
        else                                listInvoiceDetailsToTable("", "");
    }
    else{
...
void DialogInvoice::listProposalDetailsToTable(QString filter, QString field)
{
    ...
    QStringList titles;
    titles  << tr("Id") << tr("Id Produit") << tr("Nom") << tr("Prix") << tr("Quantite") << tr("Remise(%)") << tr("Total");
    ui->tableWidget->setHorizontalHeaderLabels( titles );

    //Recuperation des donnees presentent dans la bdd
    m_proposal->getProposalItemsList(ilist, "NAME", filter, field);
    // list all
    for(int i=0; i<ilist.name.count(); i++){
        ...
        // Ne pas rendre editable si c est un produit!
        // Car il il y a une liaison pour la gestion du stock
        if(ilist.idProduct.at(i) > 0){
            item_NAME->setFlags(item_NAME->flags() & (~Qt::ItemIsEditable)); // Ne pas rendre editable
...
void DialogInvoice::listInvoiceDetailsToTable(QString filter, QString field)
{
    ...
    //Recuperation des donnees presentent dans la bdd
    m_invoice->getInvoiceItemsList(m_ilist, "NAME", filter, field);
    // list all
    for(int i=0; i<m_ilist.name.count(); i++){
        ...
        item_NAME->setData(Qt::DisplayRole, m_ilist.name.at(i));
        // Ne pas rendre editable si c est un produit!
        // Car il il y a une liaison pour la gestion du stock
...
void DialogInvoice::listServiceToTable()
{
    ...
    for(unsigned int i=0; i<servicesListCommon.id.size(); i++){
        QTableWidgetItem *item_ID      = new QTableWidgetItem();
        QTableWidgetItem *item_PRICE     = new QTableWidgetItem();
        QTableWidgetItem *item_NAME     = new QTableWidgetItem();
...
void DialogInvoice::on_toolButton_rm_clicked()
{
    ...
    int ret = QMessageBox::warning(this, tr("Attention"),
                                   ...
                                   ui->tableWidget->item(m_index_tabInvoice, COL_NAME)->text()+
                                   " </b>  ?",
...
void DialogInvoice::removeProposalItems(){
    ...
    m_proposal->getProposalItemsList(ilist, "NAME", "", "");
    for(int i=0; i<ilist.name.count(); i++){
        present = false;
...
void DialogInvoice::removeInvoiceItems(){
    ...
    bool present;
    /* Procedure pour savoir s'il faut supprimer des articles */
    m_invoice->getInvoiceItemsList(ilist, "NAME", "", "");
    for(int i=0; i<ilist.name.count(); i++){
        present = false;
        //On parcourt le tableau pour savoir si l ID et present
        for (int j=ui->tableWidget->rowCount()-1; j >= 0; --j){
...
int DialogInvoice::getDiffQuantityOfItem(const int& id, int qteNew){
    ...
    for(unsigned int i=0; i< m_ilist.id.size(); i++)
...
{
   if((column == COL_PRICE) || (column == COL_QUANTITY) || (column == COL_DISCOUNT)){
...
void DialogInvoice::calcul_Total()
{
    ...
    //Cacul du reste
    float diff = m_totalPrice - ui->doubleSpinBox_partPAYMENT->value();

    //affiche la valeur
    QString val = "<h1>"+ tr("TOTAL: ") + QString::number(m_totalPrice,'f',2) + tr(" &euro; </h1>");
    if(m_DialogType == INVOICE_TYPE)val += "<h2>"+ tr("RESTE A PAYER: ") + QString::number(diff,'f',2) + tr(" &euro; </h2>");
...
/**
  ...
  */
void DialogInvoice::add_to_Table(int idProduct, QString name, float price) {
    ...
    if(cRow<=0){
        ...
        QStringList titles;
        titles  << tr("Id") << tr("IdProduct") << tr("Nom") << tr("Prix") << tr("Quantite") << tr("Remise(%)") << tr("Total");
        ui->tableWidget->setHorizontalHeaderLabels( titles );
    }
    // on increment la quantite si article deja present
    else{
        for (int j=ui->tableWidget->rowCount()-1; j >= 0; --j){
            if( (name  == ui->tableWidget->item(j, COL_NAME)->text()) &&
...
void DialogInvoice::on_toolButton_add_clicked() {
    ...
    if(ui->tabWidget_select->currentIndex() == 0){
        ...
        add_to_Table( 0, ui->tableWidget_selectService->item(m_index_tabService, 1)->text(),
                      ui->tableWidget_selectService->item(m_index_tabService, 2)->text().toFloat() );
    }
    //ou un produit
    else{
        //Si plus d article on sort
        ...
        add_to_Table( m_productView->getSelectedProductID(), m_productView->getSelectedProductName(),
                      m_productView->getSelectedProductPrice() );
    }
}
...
void DialogInvoice::createInvoiceFromProposal() {
    ...
    invoice::InvoiceItem itemInv;
    //Recuperation des donnees presentent dans la bdd
    m_proposal->getProposalItemsList(itemPro, "NAME", "", "");
