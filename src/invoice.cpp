...
bool invoice::getInvoiceListAlert(InvoiceListAlert& list) {
    ...
    QString req =   "SELECT TAB_CUSTOMERS.FIRSTNAME, TAB_CUSTOMERS.LASTNAME, TAB_INVOICES.ID, TAB_INVOICES.DATE, TAB_INVOICES.LIMIT_PAYMENTDATE, TAB_INVOICES.DESCRIPTION, TAB_INVOICES.CODE, TAB_INVOICES.DESCRIPTION, TAB_INVOICES.PRICE, TAB_INVOICES.STATE "
                    "FROM TAB_INVOICES "
...
QStringList invoice::getYearsList() {
    ...
    QString req;
    if(m_db.driverName() == "QSQLITE")
        req =   "SELECT DISTINCT YEARS FROM (SELECT strftime('%Y',TAB_INVOICES.DATE) AS YEARS FROM TAB_INVOICES) "
                "ORDER BY UPPER(YEARS) DESC;";
    else
        req =   "SELECT DISTINCT(EXTRACT(YEAR FROM TAB_INVOICES.DATE)) AS YEARS "
                "FROM TAB_INVOICES ORDER BY UPPER(DATE) DESC;";
